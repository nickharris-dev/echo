  ...
  if ( !empty($image) ) {
    ...
    foreach( $image['sizes'] as $key => $value ) {
      if ( strpos($key, $size) !== false ) {
        if ( strpos($key, 'height') !== false ) $heights[] = $value;
        elseif ( strpos($key, 'width') !== false ) $widths[] = $value;
        else $imgs[] = $value;
      }
    }

    echo ' data-media data-media-type="image" data-media-file="' . implode(',', $imgs) . '" data-media-heights="' . implode(',', $heights) . '" data-media-widths="' . implode(',', $widths) . '" data-focalpoint-x="'. $x .'" data-focalpoint-y="'. $y .'"';
  }
}
...
function html5_insert_image($html, $id, $caption, $title, $align, $url, $size, $alt ) {
  ...
  $attributes .= ' class="wp-caption ' . 'align'.esc_attr($align) . '"';
